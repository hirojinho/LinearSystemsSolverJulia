...
function solve_linear_system(A::Array{Float64,2}, b::Array{Float64,1})
    ...
    println("f: Decomposição de Cholesky.")
    ...
    elseif metodo == "e"
        ...
        return x
    elseif metodo == "f"
        println("Método de Decomposição de Cholesky selecionado.")
        x = cholesky_decomposition_solver(A_float, b_float)
        println("Solução: $x")
        return x
    else
...
# Example usage
A = [1.0 1.0 1.0; 1.0 2.0 2.0; 1.0 2.0 3.0]
b = [3.0, 5.0, 6.0]
